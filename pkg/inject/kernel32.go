...
func VirtualAllocEx(hProcess uintptr, lpAddress uintptr, dwSize uint32, flAllocationType int, flProtect int) uintptr {
	memptr, _, err := virtualAllocEx.Call(
		hProcess,
		uintptr(lpAddress),
		uintptr(dwSize),
...
func VirtualFreeEx(process uintptr, address uintptr, length int) {
	_, _, err := virtualFreeEx.Call(process, uintptr(address), uintptr(length), windows.MEM_RELEASE)
	if err != nil && err.Error() != "The operation completed successfully." {
		panic(fmt.Sprintf("[!]Error calling VirtualAlloc:\r\n%s", err.Error()))
	}
}
...
func WriteProcessMemory(hProcess uintptr, lpBaseAddress uintptr, data []byte, size uint32) (e error) {
	...
	r, _, err := writeProcessMemory.Call(hProcess,
		lpBaseAddress,
...
func VirtualProtectEx(process uintptr, address uintptr, size int, newProtect uint32) uint32 {
	var oldProtect uint32
	_, _, err := virtualProtectEx.Call(process, address, uintptr(size), uintptr(newProtect), (uintptr)(unsafe.Pointer(&oldProtect)))
	if err != nil && err.Error() != "The operation completed successfully." {
		panic(fmt.Sprintf("[!]Error calling VirtualAllocEx:\r\n%s", err.Error()))
	}
	return oldProtect
}
